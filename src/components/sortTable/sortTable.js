...
const sorting =(col) =>{
  ...
  />
        ...
        <table>
      <thead>
        <th onClick={()=> sorting("name")}><div className="heading-wrapper name">Name<LiaSortSolid className="up-down"/></div></th>
      ...
      {data.map((val) => (
          <tr key={val.index}>
            ...
            <td className="numRow">{val.number}</td>
            <td className="marketRow">${val.market_rate}</td>
            <td className="weightRow">{val.weight}%</td>
            <td className="valueRow">${val.value}</td>
          </tr>
      ))}
      </tbody>
